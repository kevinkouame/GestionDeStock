...
def about(request):
    return HttpResponse("Bienvenue sur ma première page Django -- About !")


def inserer_produit(request):

    # Appeler la procédure stockée pour récupérer les emplacements
    #with connection.cursor() as cursor:
    #    cursor.execute("{CALL [dbo].[LISTE_EMPLACEMENT]}")
    #    emplacements = cursor.fetchall()
    # Convertir les résultats en une liste de tuples (ID, Libellé)
    ...
    return render(request, 'stock/inserer_produit.html', {'form': form, 'produits_data': produits_data})


    #return render(request, 'inserer_produit.html', {'form': form, 'code_retour': code_retour})

    #render() est utilisé pour générer une réponse HTTP contenant du HTML.
    #Le template ('inserer_produit.html') est utilisé pour afficher la page.
    #Le contexte ({'form': form}) contient les données que tu veux afficher dans le template, ici le formulaire ProduitForm.

    # Vérifiez le résultat
    #code_retour = result[0] if result else None

    #return render(request, 'votre_template.html', {'code_retour': code_retour})
